...
async function P(n, { blockNumber: e, blockTag: a, data: o, to: c }) {
  ...
  try {
    ...
    const u = await A({ data: f, sender: t, urls: i }),
      { data: b } = await k(n, {
        blockNumber: e,
        blockTag: a,
        data: O([r, L([{ type: "bytes" }, { type: "bytes" }], [u, s])]),
        to: c,
      });
    return b;
  } catch (u) {
...
async function A({ data: n, sender: e, urls: a }) {
  ...
  for (let d = 0; d < a.length; d++) {
    const t = a[d],
      ...
      f = i === "POST" ? { data: n, sender: e } : void 0;
    try {
      ...
      o = new m({ body: f, details: r.message, url: t });
    }
  }
  throw o;
}
